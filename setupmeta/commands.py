        ...
        if definitions:
            ...
            for definition in sorted(definitions.values()):
                if definition.key == "versioning":
                    print("    # versioning=%s," % setupmeta.stringify(definition.value, quote=True))
                    continue
                if definition.key == "setup_requires":
                    ...
                    if not definition.value:
                        continue
                if definition.key == "download_url":
                    if version.value in definition.value:
                        definition.value = definition.value.replace(version.value, "%s")
                        print('    download_url=%s %% __version__,' % setupmeta.stringify(definition.value, quote=True))
                        continue
                if definition.key == "long_description":
                    print('    %s=open(%s).read(),' % (definition.key, setupmeta.stringify(definition.source, quote=True)))
                elif definition.key == "version":
                    print("    version=__version__,")
                elif definition.key in setupmeta.MetaDefs.all_fields:
                    print("    %s=%s," % (definition.key, setupmeta.stringify(definition.value, quote=True, indent="        ")))
        print(")")
