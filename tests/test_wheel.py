...
def test_wheel(sample_project):
    ...
    assert os.listdir(dist_folder) == ["sample-0.1.0-py2.py3-none-any.whl"]
    ...
    setupmeta.run_program("pip", "wheel", "--only-binary", ":all:", "-w", "dist", ".")
    assert sorted(os.listdir(dist_folder)) == ["sample-0.1.0-py2.py3-none-any.whl", "sample-0.1.0.dirty-py2.py3-none-any.whl"]
